...
rankhospital <- function(state, outcome, num = "best") {
    ...
    # define correct state parameter (abbreviations)
    states = data[, unique(state)]
    
    # process num parameter
    if(num == 'best') {
        param = ", decreasing = F), hospital.name][num]"
        num = 1
    } else if(num == 'worst') {
        param = ", decreasing = T), hospital.name][num]"
        num = 1
    } else {
        param = ", decreasing = F), hospital.name][num]"
    }
    ...
    if(all(outcome != outcomes)) {
        ...
        eval(parse(text=paste0("data[.('", state, "'), ][order(" ,
